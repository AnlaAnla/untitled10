...
def voc2yolo(path, save_path):
    ...
    for file in os.listdir(path):
        ...
        for obj in root.iter('object'):
            ...
            with open(os.path.join(save_path, file.replace('xml', 'txt')), 'w') as out_file:
                out_file.write(str(cls_id) + " " + " ".join(str(x) for x in bbox) + '\n')


if __name__ == '__main__':
    # 这里要改成自己数据集路径的格式
    path = r"D:\BaiduNetdiskDownload\yolo(prizm+mosaic)\label2"
    save_path = r"D:\BaiduNetdiskDownload\yolo(prizm+mosaic)\label2_yolo"
    voc2yolo(path, save_path)
    print('end')
