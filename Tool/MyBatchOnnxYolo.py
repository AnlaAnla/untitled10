...
'''用法示例
# 初始化模型
model = MyBatchOnnxYolo(r"yolo_handcard01.onnx") # 或者你的 .pt / .engine 模型

# 准备图片列表 (可以是路径, PIL Image, 或 NumPy 数组)
img_paths = [
    r"D:\Code\ML\Embedding\img_vec\image\--IAAOSw8uJnlgrm.jpg",
    r"path/to/your/image2.jpg",
    # ... 更多图片
]
images_to_process = img_paths # 或者加载成 PIL/NumPy 列表

# 批处理预测
model.predict_batch(images_to_process)

# 检查或获取单个结果
num_images = model.get_batch_size()
if num_images > 0:
    # 检查第一张图片是否有 card (cls_id=0)
    has_card_img0 = model.check(index=0, cls_id=0)
    print(f"Image 0 has card: {has_card_img0}")

    # 获取第一张图片中最大的 card 截图
    if has_card_img0:
        max_card_img0 = model.get_max_img(index=0, cls_id=0)
        # 可以显示或保存 max_card_img0 (它是 RGB NumPy 数组)
        # import matplotlib.pyplot as plt
        # plt.imshow(max_card_img0)
        # plt.axis('off')
        # plt.show()
...
for i in range(num_images):
    print(f"\n--- Processing Image {i} ---")
    if model.check(index=i, cls_id=0):
        print(f"  Found card (cls_id=0).")
        max_card_crop = model.get_max_img(index=i, cls_id=0)
        print(f"  Cropped max card shape: {max_card_crop.shape}")
        # 在这里处理 max_card_crop, 比如保存或进一步分析
        # cv2.imwrite(f"output_card_{i}.jpg", cv2.cvtColor(max_card_crop, cv2.COLOR_RGB2BGR))
    else:
        print(f"  No card (cls_id=0) found.")

    # 可选：显示带标注的单个结果
    # model.show_result(index=i)
...
class MyBatchOnnxYolo:
    ...
    def __init__(self, model_path: str, task: str = 'detect', verbose: bool = False):
        """
        初始化 YOLO 模型。

        Args:
            model_path (str): YOLO 模型文件的路径 (.pt, .onnx, .engine等)。
            task (str): 模型任务类型 ('detect', 'segment')。
            verbose (bool): 是否显示 Ultralytics 的详细输出。
        """
        # 加载yolo model
