...
if __name__ == '__main__':
    ...
    label_path = r"C:\Code\ML\Image\angle_data\train\label_r"
    for img_name in os.listdir(dir_path):
        img_path = os.path.join(dir_path, img_name)
        angel = 90 - get_angle(img_path)
        save_path = os.path.join(label_path , img_name[:-4] + ".txt")

        print(f"{save_path}: {angel}")

        # with open(save_path, 'w') as f:
        #     f.write(str(int(angel)))
        # correct_angle(os.path.join(dit_path, img_path))
